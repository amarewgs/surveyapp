				...
				<div class="span12">
					<div class="box" style="display: inline-block;">
						<div class="box-header">
							...
							<h5>Questionnaire completed by ${user.fullName}</h5>
						...
						<spring:url var = "action" value='/public/auth/questionnaire/save' />
							<%-- <form:form method="post" action="${action}" class="form" commandName="questionGroupForm" id="questionGroupForm"> --%>
								<div class="container span12">
									...
									<c:if test="${null != questionGroupForm }">
												<div style="display: inline-block; border-bottom: 1px solid silver; margin-bottom: 4px; width: 90%; vertical-align: top;">
												...
												<!-- TODO: Add categorizing by question group -->
														...
														<c:if test="${questionForm.question.questionType == 'OPEN_ENDED' }">
															<div class="controls">
																<textarea id="userAnswerOpenEnded${questionRow.index }" name="userAnswerForm[${questionRow.index }].detailAnswer" class="span5"
																	type="text" value="${questionForm.answerSheetForm.detailAnswer }" placeholder="User Open ended text">${questionForm.answerSheetForm.detailAnswer }</textarea>
															</div>
														</c:if>
															...
															<label for="challengeQuestion" class="control-label">
																...
																<div class="controls">
																	<textarea id="userAnswerComment${questionRow.index}" name="userAnswerForm[${questionRow.index }].comment" class="span5"
																		type="text" value="${questionForm.answerSheetForm.comment }" placeholder="User can give comment">${questionForm.answerSheetForm.comment }</textarea>
																</div>
															</div>
														</c:if>
														<c:if test="${questionForm.question.attachmentEnabled }">
															...
															<label class="control-label">Attachment:</label>
															 <div class="controls">
															 	<c:if test="${questionForm.answerSheetForm.attachmentPath != null}">
																		<a href="<spring:url value="/admin/user/attachments/${questionForm.answerSheetForm.userAnswerId }" htmlEscape="true" />"><i class="btn-icon-only icon-download"></i>${questionForm.answerSheetForm.attachmentPath}</a>
																</c:if>
																<c:if test="${questionForm.answerSheetForm.attachmentPath == null}">
																	<span style="font-style: italic; font-size: 12px;">no attachments have been found</span>
																</c:if>
															</div>
															</div>
														</c:if>
							...
							<footer id="submit-actions" class="form-actions">
								...
								<button type="button"
									onclick="window.location='<spring:url value="/admin/questionnairesCompleted/${user.id}" htmlEscape="true" />'" class="btn"
